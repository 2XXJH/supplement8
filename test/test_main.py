...
def test_should_get_request_error():
    url = "https://jsonplaceholder.typicode.com/invalid_url"
    with pytest.raises(Exception):
        get_request(url)

@patch('requests.get')
def test_should_get_beeceptor_info(mock_get):
...
@patch('requests.post')
def test_should_post_beeceptor_hello(mock_post):
    mock_response = requests.Response()
    mock_response.status_code = 200
    mock_response._content = b'{"hello": "world"}'
    mock_post.return_value = mock_response

    response_json = post_beeceptor_hello()
